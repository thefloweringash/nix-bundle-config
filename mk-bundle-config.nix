...
let
  ...
  getAllGemDeps = gemConfig:
    let
      gemDeps = (k: v:
         (v.pkgConfigLibs or []) ++ (v.buildInputs or []) ++ (lib.attrValues (v.dirConfigLibs or {}))
       );
    in
      lib.concatLists (lib.mapAttrsToList gemDeps gemConfig);

  mkGcRoot = name: drv: writeText "${name}-gc-root" ''
    # Pointer into the nix store to cause the target to be retained.

    # The file indirection, instead of a direct link, prevents editors
    # and search tools from navigating to or indexing the dependencies
...
in {
  ...
  gemDepsGcRoot = mkGcRoot "gem-deps" allGemDepsEnv;
